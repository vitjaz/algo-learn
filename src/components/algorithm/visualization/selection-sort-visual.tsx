...
interface SelectionSortVisualProps {
  step: SelectionSortStep;
}

export function SelectionSortVisual({ step }: SelectionSortVisualProps) {
  const t = useTranslations();
  const tv = useTranslations("algorithm.visualization.legend");
  const { array, range, minIndex, comparing, swapping, sortedIndices, type } =
    step;
    ...
    <div className="flex flex-col items-center gap-6 p-4">
      ...
      {type !== "initial" && type !== "sorted" && (
        <div className="text-xs text-muted-foreground">
          ...
          {minIndex !== null && (
            <span className="ml-3">
              {t("algorithm.visualization.min")}: {array[minIndex]}
            </span>
          )}
        </div>
      )}
      ...
      <div className="text-center text-sm p-3 bg-muted rounded-lg max-w-md">
        <span>
          {t(`algorithms.selection-sort.steps.${type}`, step.descriptionParams)}
        </span>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-center gap-3 text-xs">
        ...
        <div className="flex items-center gap-1">
          ...
          <span className="text-muted-foreground">{tv("minimum")}</span>
        </div>
        <div className="flex items-center gap-1">
